...
while ($true) {
    ...
    elseif ($Pool -eq "miningpoolhub") {
        try {
			...
			$TempBalanceData = ((((Invoke-WebRequest ($APIUri + $Wallet) -TimeoutSec 15 -UseBasicParsing -Headers @{"Cache-Control" = "no-cache"}).content | ConvertFrom-Json).getuserallbalances).data | Where {$_.coin -eq "litecoin"}) 
        }
        catch {  }#.confirmed
    }
    else {
