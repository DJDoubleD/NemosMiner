...
$Path = ".\Bin\NVIDIA-ccminerPigeon\ccminer.exe"
...
$Commands = [PSCustomObject]@{
    ...
    #"x16r" = " -r 0 -d $($Config.SelGPUCC) -i 20" #X16r
    "x16s" = " -d $($Config.SelGPUCC) -i 21" #X16s (fastest)
}
...
$Commands | Get-Member -MemberType NoteProperty | Select -ExpandProperty Name | ForEach {
    [PSCustomObject]@{
        Type = "NVIDIA"
        Path = $Path
        Arguments = "-b $($Variables.NVIDIAMinerAPITCPPort) -R 1 --donate 0 -a $_ -o stratum+tcp://$($Pools.(Get-Algorithm($_)).Host):$($Pools.(Get-Algorithm($_)).Port) -u $($Pools.(Get-Algorithm($_)).User) -p $($Pools.(Get-Algorithm($_)).Pass)$($Commands.$_)"
